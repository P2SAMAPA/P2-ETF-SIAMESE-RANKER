...
def load_output(module: str) -> Optional[Dict]:
    ...
    local = f"{module}_module/output.json"
    if os.path.exists(local):
        with open(local) as f:
            return json.load(f)
    try:
        from core.hf_storage import pull_latest_ranking
...
def load_backtest(module: str, mode: str) -> Optional[Dict]:
    ...
    if os.path.exists(local):
        with open(local) as f:
            return json.load(f)
    try:
        from core.hf_storage import pull_backtest_results
...
def load_signal_history(module: str) -> Optional[pd.DataFrame]:
    ...
    try:
        from core.hf_storage import pull_signal_history
...
def render_sidebar():
    with st.sidebar:
            ...
            with st.spinner("Running pipeline..."):
                try:
                    ...
                    if module_choice in ["Both", "FI / Alts"]:
                        from fi_module.predict import main as fi_main
                        fi_main(run_backtest=run_backtest, force_lgbm=force_lgbm)

                    if module_choice in ["Both", "Equity Sectors"]:
                        from equity_module.predict import main as eq_main
        ...
        for module in ["fi", "equity"]:
            p = f"{module}_module/output.json"
            if os.path.exists(p):
                with open(p) as f:
                    d = json.load(f)
