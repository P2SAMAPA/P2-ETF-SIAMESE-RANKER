"""
equity_train.py
Equity Sectors module — training entrypoint.
Run: python equity_train.py [--lgbm]
"""
...
def main(force_lgbm: bool = False):
    ...
    logger.info(f"=== Equity Training | Universe: {universe} ===")
    ...
    for h, (_, backend) in models.items():
        ...
        path = os.path.join(model_base_path, f"equity_{backend}_h{h}{ext}")
        if os.path.exists(path):
            push_model_weights(path, "equity", h, backend)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Equity module")
