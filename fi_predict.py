"""
fi_predict.py
FI / Commodities module — inference + backtest entrypoint.
Run: python fi_predict.py [--no-backtest] [--lgbm]
"""
...
def main(run_backtest: bool = True, force_lgbm: bool = False):
    ...
    output_fixed = format_output(
        module="fi", ranking=result["ranking"], conviction=result["conviction"],
    ...
    save_output(combined, "outputs/fi_output.json")
    save_output(combined, "fi_output.json")  # root-level copy for Streamlit fallback
    ...
    if run_backtest:
        ...
        push_ranking_output(combined, "fi")
        push_backtest_results(fs_bt, "fi", "fixed_split")
        if sw_bt:
            push_backtest_results(sw_bt, "fi", "shrinking_window")
        push_signal_history(history, "fi")
        # Also save locally so Streamlit can display without HF token
        import pandas as _pd
        os.makedirs("outputs", exist_ok=True)
        _pd.DataFrame(history).to_csv(f"outputs/signal_history_fi.csv", index=False)
        _pd.DataFrame(history).to_csv(f"signal_history_fi.csv", index=False)

    logger.info(f"=== FI Done | Top: {output_fixed['top_pick']}  "
                f"Conviction: {output_fixed['top_conviction']:.1%} ===")
    return combined


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FI module prediction")
